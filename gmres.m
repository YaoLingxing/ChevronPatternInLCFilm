function [x, error, total_iters] = gmres(x0, b, atv, params,...
                           sdc_t,sdc_dt,sdc_A,sdc_dfdy,sdc_n,sdc_m)
...
if kmax==0
  x=b;
  error=[norm(feval(atv,x0,sdc_t,sdc_dt,sdc_A,sdc_dfdy,sdc_n,sdc_m)-b), ...
         norm(feval(atv,b,sdc_t,sdc_dt,sdc_A,sdc_dfdy,sdc_n,sdc_m)-b)];
  total_iters=0;
  return;
end;
...
while((rho > errtol) & (k < kmax))
    k=k+1;
    v(:,k+1)=feval(atv,v(:,k),sdc_t,sdc_dt,sdc_A,sdc_dfdy,sdc_n,sdc_m);
    normav=norm(v(:,k+1));
%
